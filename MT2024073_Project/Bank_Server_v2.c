...
int apply_for_loan(int client_socket) {
    ...
    if (loan_amount <= 0) {
        send(client_socket, "Invalid loan amount.\n", strlen("Invalid loan amount.\n"), 0);
        return 0;
    }

    // Generate a unique loan ID
    loan_id_num = get_next_loan_id();
    snprintf(loan_id, sizeof(loan_id), "loan_%d", loan_id_num);

    // Open the loan database file in append mode
    FILE *file = fopen(LOAN_DB, "a");
    if (!file) {
    ...
    // Write the loan details to the file
    fprintf(file, "%s %d %.2lf\n", loan_id, logged_in_cust_id, loan_amount);
    fflush(file);  // Ensure data is written to disk
    ...
    snprintf(success_msg, sizeof(success_msg), "Loan applied successfully! Your Loan ID is %s.\n", loan_id);
